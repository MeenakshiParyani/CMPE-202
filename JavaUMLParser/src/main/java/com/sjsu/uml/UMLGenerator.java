...
import com.sjsu.parser.JavaUMLParser;
...
public class UMLGenerator {
	...
	private void getClassOrInterfaceUML(StringBuilder builder, ClassOrInterfaceDeclaration classOrInterface) {
		...
		if(classOrInterface.isInterface())
			...
			exntendedTypes.stream().forEach(node -> builder.append(UMLRelationshipType.EXTENDS.getSymbol() + node.getNameAsString()));
			if(!implementedTypes.isEmpty()){
				builder.append(UMLRelationshipType.IMPLEMENTS.getSymbol());
	 ...
	 */
	private void checkAndAddRelationships(String containingClass, String containedClass, UMLRelationship relationship) {
		...
		for(MethodDeclaration method : filteredMethods){
			...
			{	
				...
				if(symbolReference.isSolved()){
					...
					//else
					checkAndAddRelationships(classOrInterface.getNameAsString(), parameter.getType().toString(), UMLRelationshipType.USES);
				}
			});
		}
	}
